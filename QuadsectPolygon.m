...
if sum(topLeft) ~= 1
    if sum(topLeft) == 0
        while sum(topLeft) ~= 1 % too conservative
            yratio = yratio + 0.01;
            xratio = xratio + 0.01;
            topLeft = (yCor < ymax*yratio & xCor < xmax*xratio);
        end
    ...
    elseif sum(botLeft) > 1 % too liberal
        while sum(botLeft) ~= 1
            ...
            elseif sum(xCor < xval) == 0
                ...
                botLeft = (yCor > ymax*yratio & xCor < xmax*xratio);
            end
        end
    end
end
    ...
    if sum(topRight) == 0
        ...
        if sum(yCor < ymax*0.5) == 0
            ...
            while sum(topRight) ~= 1 % too conservative
                minNow = minSorty(1:i);
                yVals = ismember(yCor, minNow);
                xratio = xratio + 0.01;
                topRight = (yVals & xCor > xmax*xratio);
                i = 1 + 1;
            end
        else
...
topCoord = min(find(B{1,1}(:,2) == floor(topMidDist)));

plot((B{1,1}(topCoord,2)),(B{1,1}(topCoord,1)), 'b.','MarkerSize', 20);

% Bottom Coordinate
botMidDist = (xCor(botRight) + xCor(botLeft))/2;
botCoord = max(find(B{1,1}(:,2) == floor(botMidDist)));

botCorX = (B{1,1}(botCoord,2));
botCorY = (B{1,1}(botCoord,1));
plot((B{1,1}(botCoord,2)),(B{1,1}(botCoord,1)), 'b.','MarkerSize', 20);

% Left Coordinate
leftMidDist = (yCor(topLeft) + yCor(botLeft))/2;
leftCoord = min(find(B{1,1}(:,1) == floor(leftMidDist)));

leftCorX = (B{1,1}(leftCoord,2));
leftCorY = (B{1,1}(leftCoord,1));

plot((B{1,1}(leftCoord,2)),(B{1,1}(leftCoord,1)), 'b.','MarkerSize', 20);

% Right Coordinate
rightMidDist = (yCor(topRight) + yCor(botRight))/2;
rightCoord = max(find(B{1,1}(:,1) == floor(rightMidDist)));

plot((B{1,1}(rightCoord,2)),(B{1,1}(rightCoord,1)), 'b.','MarkerSize', 20);

%%  THIS IS FUCKED UP

% % Center top line
% line([pixelInfo.Centroid(1) pixelInfo.Centroid(2)],[(B{1,1}(topCoord,2)) (B{1,1}(topCoord,1))], 'Color','r','LineWidth',2);
% % Center bottom line
% line([pixelInfo.Centroid(1) pixelInfo.Centroid(2)],[botCorX  botCorY], 'Color','r','LineWidth',2);
% % Center left line
% line([pixelInfo.Centroid(1) pixelInfo.Centroid(2)],[leftCorY leftCorX], 'Color','g','LineWidth',2);
% % Center right line
% line([pixelInfo.Centroid(1) pixelInfo.Centroid(2)],[(B{1,1}(rightCoord,2)) (B{1,1}(rightCoord,1))], 'Color','g ','LineWidth',2);
% 
% 
% 
...
q1_coord2 = [(B{1,1}(topCoord,2));(B{1,1}(topCoord,1))];
...
q1_ytest = yCor < quadrant_1_Ycoords(3);
q1_xtest = (xCor > quadrant_1_Xcoords(3) | xCor < quadrant_1_Xcoords(3)) & xCor < quadrant_1_Xcoords(1);
...
for q1i2 = 1:length(Quad1_coord_indices)
    quadrant_1_Xcoords(q1r + q1i2) = xCor(Quad1_coord_indices(q1i2));
    quadrant_1_Ycoords(q1r + q1i2) = yCor(Quad1_coord_indices(q1i2));
end
